...
import { lte, parse } from "../../deps/semver.ts";
...
export const lessOrEqual = {
  command: "lte <v1> <v2>",
  describe: "Return 1 if v1 is less than or equal to v2, 0 otherwise",
  ...
  async handler(args: Arguments & IContext) {
    ...
    const result = lte(version1, version2) ? 1 : 0;
    await printComparison(args, v1, v2, result, "lte", jsonOutput);
  },
};
